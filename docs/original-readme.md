...
RAGPilot is a hackathon-ready Adaptive Multi-RAG Orchestrator. It accepts mixed inputs, analyzes their structure, selects the best retrieval path, and answers questions with route explanations, citations, generated SQL, and a lightweight graph view.

This is intentionally not a "chat with PDFs" clone. RAGPilot composes reusable retrieval modules:
...
RAGPilot can run without `OPENAI_API_KEY` for local smoke tests by using deterministic lexical retrieval and SQL heuristics. For the strongest demo, set the key.

## Demo Flow

1. Upload `examples/demo_notes.md` and `examples/sales.csv`.
2. Ask: `What is RAGPilot designed to do?`
3. Ask: `What is the total revenue?`
4. Ask: `What relationships exist between RAGPilot, SQL RAG, and Semantic RAG?`
...
For the adaptive text demo, upload `examples/mixed_college_data.txt`. RAGPilot will segment the text, convert reliable table-like sections into SQLite tables, activate keyword lookup for person/name queries, activate graph retrieval for relationship questions, and show the selected RAG modes in the dashboard.
...
- `RAGPILOT_DATA_DIR`: optional local storage directory; defaults to `backend/data`.
